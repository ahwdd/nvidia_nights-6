...
function AboutEvent() {
  ...
  return (
    <div className="">
      <svg
        className="absolute hidden right-0 w-[35px] lg:w-[62px]"
        width="62"
        height="177"
        viewBox="0 0 62 177"
        fill="none"
      ...
      <div className="flex flex-col gap-5 justify-around items-center w-full">
        ...
        <div className="w-full text-center font-normal relative max-w-[52rem] text-xl rtl:text-lg text-[1.1rem]"
          dangerouslySetInnerHTML={{ __html: `<span class="">${t("title1")}</span>
          <a href="https://www.nvidia.com/" class="w-fit relative">
            <span class="">${t("span")}</span><span class="w-4/5 h-0.5 bg-mainGreen absolute bottom-0 left-1/2 -translate-x-1/2"/></a>
          <span class="">${t("title2")}</span>`
          }}
        />

        <Image
          alt="mask"
          className="absolute hidden -z-10 mx-14 w-[310px] h-[200px]  lg:w-[510px] lg:h-[310px]"
          src={maskAbout}
        />
      </div>
    </div>
